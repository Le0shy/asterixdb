...
public class VectorClusteringDataFrame extends VectorClusteringNSMFrame implements IVectorClusteringDataFrame {
    ...
    public VectorClusteringDataFrame(ITreeIndexTupleWriter tupleWriter, int centroidDimensions) {
    ...
    public FrameOpSpaceStatus hasSpaceInsert(ITupleReference tuple) throws HyracksDataException {
        ...
        if (bytesRequired + slotManager.getSlotSize() <= buf.capacity()
                - buf.getInt(ITreeIndexFrame.Constants.FREE_SPACE_OFFSET)
                - (buf.getInt(ITreeIndexFrame.Constants.TUPLE_COUNT_OFFSET) * slotManager.getSlotSize())) {
