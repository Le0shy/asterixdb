...
public final class KMeansVectorCodec {
    ...
    public static final class ListVectorDecoder {
        ...
        /**
         * Decodes the vector column, or returns {@code null} when the list holds an element that is not a
         * number -- a string, an object, a null. {@link VectorListDecoder} renders those as NaN, and a NaN
         * poisons every distance it touches, so such a row cannot participate in clustering.
         * <p>
         * Returning null rather than raising is the caller's cue to <em>skip the row with a warning</em>,
         * matching what {@code nearest_centroid} already does on the labeling side. The desugar's width filter
         * cannot catch this case: it checks that the value is an array of the declared length, which a vector
         * with one bad element still is.
         * <p>
         * A genuine NaN in the source data is skipped on the same grounds -- it is equally unusable.
         */
