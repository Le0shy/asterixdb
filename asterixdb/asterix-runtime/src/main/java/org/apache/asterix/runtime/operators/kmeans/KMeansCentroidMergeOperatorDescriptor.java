...
public class KMeansCentroidMergeOperatorDescriptor extends AbstractSingleActivityOperatorDescriptor {
    ...
    public IOperatorNodePushable createPushRuntime(IHyracksTaskContext ctx,
        ...
        return new AbstractUnaryInputUnaryOutputOperatorNodePushable() {
            ...
            private final Map<Integer, List<Partial>> partialsByIter = new HashMap<>();
                ...
                for (int i = 0; i < tupleCount; i++) {
                    ...
                    if (kind == KMeansLoopIO.KIND_END) {
                        ...
                        if (ends == nParticipants) {
                            emitCentroids(iter);
                            partialsByIter.remove(iter);
                            endsByIter.remove(iter);
                        }
                    } else {
                        double count = DoublePointable.getDouble(tuple.getFieldData(4), tuple.getFieldStart(4));
                        double[] sum = KMeansLoopIO.readRawVector(tuple.getFieldData(5), tuple.getFieldStart(5),
                                tuple.getFieldLength(5));
                        partialsByIter.computeIfAbsent(iter, k -> new ArrayList<>())
                                .add(new Partial(part, seq, (long) count, sum));
                    }
                }
            }
            ...
            private void emitCentroids(int iter) throws HyracksDataException {
                List<Partial> partials = partialsByIter.get(iter);
                if (partials != null && !partials.isEmpty()) {
                    partials.sort(MERGE_ORDER);
                    int maxSeq = 0;
                    ...
                    for (Partial p : partials) {
                        ...
                        weights[p.seq] += p.count;
                        double[] sum = sums[p.seq];
                        if (sum == null) {
                            sums[p.seq] = p.sum.clone();
                        } else {
                            for (int d = 0; d < Math.min(sum.length, p.sum.length); d++) {
                                sum[d] += p.sum[d];
                            }
                        }
                    }
