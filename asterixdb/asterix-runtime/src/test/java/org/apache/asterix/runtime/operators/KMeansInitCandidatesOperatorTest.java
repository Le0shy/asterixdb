...
public class KMeansInitCandidatesOperatorTest {
    ...
    @Test
    public void storeThenScoreEmitsLocalTopL() throws Exception {
        ...
        KMeansInitCandidatesOperatorDescriptor op = new KMeansInitCandidatesOperatorDescriptor(spec, VEC_REC_DESC,
                KMeansInitCandidatesOperatorDescriptor.Mode.ROUND, 4, 0, 0, false);

        List<IActivity> activities = collectActivities(op);
        IRecordDescriptorProvider rdp = recordDescProvider();

        // Store the partition: A(0,0) B(1,0) C(8,8) D(9,8) E(0,1) F(8,9).
        IOperatorNodePushable store = activities.get(0).createPushRuntime(ctx, rdp, 0, 1);
        store.getInputFrameWriter(0).open();
        store.getInputFrameWriter(0).nextFrame(
                vectorsFrame(ctx, new double[][] { { 0, 0 }, { 1, 0 }, { 8, 8 }, { 9, 8 }, { 0, 1 }, { 8, 9 } }));
        store.getInputFrameWriter(0).close();

        // Materialize the pool = [ (0,0) ] via the pool Store activity.
        IOperatorNodePushable poolStore = activities.get(1).createPushRuntime(ctx, rdp, 0, 1);
        poolStore.getInputFrameWriter(0).open();
        poolStore.getInputFrameWriter(0).nextFrame(vectorsFrame(ctx, new double[][] { { 0, 0 } }));
        poolStore.getInputFrameWriter(0).close();

        // Score: source activity; expects the pool echo then the local top-4 by d2, ties keep earliest,
        // all as [kind, partition, seq, score, vector] envelopes.
        IOperatorNodePushable score = activities.get(2).createPushRuntime(ctx, rdp, 0, 1);
        List<double[]> out = collectOutput(score, true);

        // Partition 0 echoes the pool through first (the output stream IS the next round's pool), then the
        // local candidates. d2 from (0,0): D=145, F=145, C=128, B=1, E=1; A itself is excluded (d2 = 0).
        // Each decoded row is [kind, partition, seq, score, v0, v1].
        Assert.assertEquals(5, out.size());
        Assert.assertArrayEquals(new double[] { 0, 0, 0, 0, 0, 0 }, out.get(0), 0.0);
        Assert.assertArrayEquals(new double[] { 1, 0, 3, 145, 9, 8 }, out.get(1), 0.0);
        Assert.assertArrayEquals(new double[] { 1, 0, 5, 145, 8, 9 }, out.get(2), 0.0);
        Assert.assertArrayEquals(new double[] { 1, 0, 2, 128, 8, 8 }, out.get(3), 0.0);
        Assert.assertArrayEquals(new double[] { 1, 0, 1, 1, 1, 0 }, out.get(4), 0.0);
    }

    @Test
    public void finalizeReLimitsGloballyAndUnwraps() throws Exception {
        IHyracksTaskContext ctx = TestUtils.create(32768);
        JobSpecification spec = new JobSpecification();
        // FINALIZE: normalize the envelope intake to pool + global top-2, emit plain vectors.
        KMeansInitCandidatesOperatorDescriptor op = new KMeansInitCandidatesOperatorDescriptor(spec, VEC_REC_DESC,
                KMeansInitCandidatesOperatorDescriptor.Mode.FINALIZE, 2, 0, 0, true);

        List<IActivity> activities = collectActivities(op);
        IRecordDescriptorProvider rdp = recordDescProvider();

        // The vector input is unused in finalize mode; store a dummy row.
        IOperatorNodePushable store = activities.get(0).createPushRuntime(ctx, rdp, 0, 1);
        store.getInputFrameWriter(0).open();
        store.getInputFrameWriter(0).nextFrame(vectorsFrame(ctx, new double[][] { { 0, 0 } }));
        store.getInputFrameWriter(0).close();

        // Envelope intake, deliberately SCRAMBLED (arrival order must not matter): one pool row and four
        // candidates from two origin partitions, with a score tie broken by (partition ASC, seq ASC).
        IOperatorNodePushable poolStore = activities.get(1).createPushRuntime(ctx, rdp, 0, 1);
        poolStore.getInputFrameWriter(0).open();
        poolStore.getInputFrameWriter(0)
                .nextFrame(envelopesFrame(ctx,
                        new double[][] {
                                ...
                                { 1, 1, 1, 9, 2, 2 }, // candidate, tied top score, LATER partition
                                { 1, 0, 2, 1, 4, 4 }, // candidate, weak
                                { 0, 0, 0, 0, 0, 0 }, // the pool member, arriving mid-stream
                                { 1, 0, 7, 9, 3, 3 }, // candidate, tied top score, partition 0 → wins the tie
                                { 1, 1, 0, 5, 1, 1 } })); // candidate, middling
        poolStore.getInputFrameWriter(0).close();

        IOperatorNodePushable score = activities.get(2).createPushRuntime(ctx, rdp, 0, 1);
        List<double[]> out = collectOutput(score, false);

        // Pool first (by seq), then global top-2: (9, p0, s7) before (9, p1, s1); (5,...) and (1,...) cut.
        Assert.assertEquals(3, out.size());
        Assert.assertArrayEquals(new double[] { 0, 0 }, out.get(0), 0.0);
        Assert.assertArrayEquals(new double[] { 3, 3 }, out.get(1), 0.0);
        Assert.assertArrayEquals(new double[] { 2, 2 }, out.get(2), 0.0);
    }
